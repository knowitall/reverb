...
public class ArgLearner extends Extractor<ChunkedExtraction, ChunkedArgumentExtraction> {
	...
	public ArgLearner(Mode mode) {
	    ...
	    if(mode==Mode.LEFT){
		    ...
		    arg1leftboundclassifier = new ArgSubstructureClassifier(mode, featuregeneratorsub);
	    }
	    else{
		    ArgSubstructureFeatureGenerator featuregeneratorsub = new ArgSubstructureFeatureGenerator(mode);
	    	arg2leftboundclassifier = new ArgLocationClassifier(mode);
	    	arg2rightboundclassifier = new ArgSubstructureClassifier(mode, featuregeneratorsub);
	    }
	...
	private ChunkedArgumentExtraction getArg1(ChunkedExtraction predicate) {
    	...
    	double[] classifierresults = arg1rightboundclassifier.getArgBound(predicate);
	...
	private double[] getArg1LeftBound(ChunkedExtraction predicate, int rightbound) {
		double[] resultsclassifier  = arg1leftboundclassifier.getArgBound(predicate, rightbound);
	...
	private double[] getArg2RightBound(ChunkedExtraction predicate, int leftbound) {
		double[] resultsclassifier = arg2rightboundclassifier.getArgBound(predicate, leftbound);
    	return resultsclassifier;
	}
