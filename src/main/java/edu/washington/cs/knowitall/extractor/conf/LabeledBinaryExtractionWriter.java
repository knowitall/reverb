...
/***
 * Used for writing <code>LabeledBinaryExtraction</code> objects to plain text. See
 * the documentation for <code>LabeledBinaryExtractionReader</code> for details on the
 * format used.
 * @author afader
 *
 */
public class LabeledBinaryExtractionWriter {
	...
	public void writeExtraction(LabeledBinaryExtraction extr) throws IOException {
		...
		out.println(arg1.getRange().getStart() + " " + arg1.getRange().getLength());
		
		ChunkedExtraction rel = extr.getRelation();
		out.println(rel.getTokensAsString());
		out.println(rel.getRange().getStart() + " " + rel.getRange().getLength());
		
		ChunkedArgumentExtraction arg2 = extr.getArgument2();
		out.println(arg2.getTokensAsString());
		out.println(arg2.getRange().getStart() + " " + arg2.getRange().getLength());
