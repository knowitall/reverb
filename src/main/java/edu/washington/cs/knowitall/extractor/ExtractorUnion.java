...
/***
 * Takes the union of the output of multiple extractors. The output of this extractor
 * is the output of the others concatenated together.
 * @author afader
 *
 * @param <S>
 * @param <T>
...
public class ExtractorUnion<S, T> extends Extractor<S, T> {
    ...
    /**
     * Constructs a new extractor that returns the union of the output from
     * each of the given extractors. 
     * @param extractors
     */
    ...
    protected Collection<T> extractCandidates(S source) 
        ...
        if (extractors == null || extractors.size() == 0) {
            ...
            ArrayList<T> results = 
            	new ArrayList<T>(extractors.size());
