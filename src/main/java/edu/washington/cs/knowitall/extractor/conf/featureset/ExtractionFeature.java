 ...
 * that picks a particular range and 
 * applies a test to all indices within
 * that range.
 * 
 * For example, the feature `return true if arg2 contains token "fish"`
 * 	would be implemented by having rangeToExamine() return arg2.getRange()
 *  and testAtIndex() returning sentence.getToken(index).equalsIgnoreCase("fish");
 * 
 * @author Rob
 */
public abstract class ExtractionFeature implements Predicate<ChunkedBinaryExtraction> {
	...
	protected abstract boolean testAtIndex(Integer index, ChunkedSentence sentence);


	/**
	 * Implements the following naive algorithm for
	 * locating the head verb within a verb phrase:<br><br>
	 * 
	 ...
	 * 
	 * 
	 * If exception, throw an IllegalArgumentException if no verb in the relation.
	 * If !exception, return null to the client if no verb in the relation.
	 * 
	 * @param relation
	...
	public static Integer indexOfHeadVerb(ChunkedExtraction relation, boolean exception) {
		...
		for (int i = relation.getLength()-1;
			 i >= 0; 
			 i--) {
		...
		if (exception) throw new IllegalArgumentException(relation.toOpenNlpFormat());
		else return null;
	}
