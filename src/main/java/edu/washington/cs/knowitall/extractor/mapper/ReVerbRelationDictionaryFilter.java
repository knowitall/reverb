...
/***
 * Filters relations based on how many distinct arg2 values it takes in a 
 * large corpus. These frequencies must be pre-computed and stored in a flat
 * file in the tab-delimited format (#arg2s, relation). The relations should be
 * normalized using the <code>VerbalRelationNormalizer</code> class.
 * 
 * By default, this class searches the classpath for a file called <code>freq_rel.txt.gz</code>
 * and reads the relations with at least 20 distinct arg2s from it. 
 * @author afader
 *
 */
public class ReVerbRelationDictionaryFilter extends FilterMapper<ChunkedExtraction> {
    ...
    private static final int defaultMinFreq = 20;
    private NormalizedRelationDictionaryFilter filter;
    
    /**
     * Constructs a new dictionary filter from the data in <code>in</code>. This data should
     * be in the tab-delimited format (#arg2, relation). 
     *  
    ...
    /**
     * Constructs a new dictionary filter using the data in the file <code>freq_rel.txt.gz</code>, which
    ...
    /**
     * Constructs a new dictionary filter using the data in the file <code>freq_rel.txt.gz</code>, and
    ...
    private InputStream getDefaultRelationStream() throws IOException {
        InputStream in = ResourceUtils.loadResource(relationDictFile, ReVerbRelationDictionaryFilter.class);
        // InputStream in = ReVerbRelationDictionaryFilter.class.getClassLoader().getResourceAsStream(relationDictFile);
        if (in != null) {
            return new GZIPInputStream(in);
        } else {
            throw new IOException("Could not load file " + relationDictFile + " from classpath.");
        }
    }
