...
public abstract class TokenFeature extends ExtractionFeature {
	...
	/**
	 * Get a feature that fires if any element of tokens is present
	 * within arg2.
	 * @param posTags
	 * @return
	 */
	...
	/**
	 * Get a feature that fires if any element of tokens is present
	 * at the index immediately before arg1
	 * @param posTags
	 * @return
	 */
	public static ExtractionFeature rightBeforeArg1(Collection<String> tokens) {
		return new TokenFeature(tokens) {
			...
			protected Range rangeToExamine(ChunkedBinaryExtraction cbe) {
				...
				else return Range.fromInterval(index, index+1);
			}
		};
	}
	...
	/**
	 * Get a feature that fires if any element of tokens is present
	 * at the index of the relation's head verb.
	 * @param posTags
	 * @return
	 */
	public static ExtractionFeature relationHeadVerb(Collection<String> tokens) {
		return new TokenFeature(tokens) {
			...
			protected Range rangeToExamine(ChunkedBinaryExtraction cbe) {
				...
				if (index == null) return Range.EMPTY;
				else return Range.fromInterval(index, index+1);
			}
		};
	}
	...
	/**
	 * Get a feature that fires if any element of tokens is present
	 * at the index immediately after arg2.
	 * @param posTags
	 * @return
	 */
	...
	public static ExtractionFeature anywhereBeforeArg1(Collection<String> tokens) {
		return new TokenFeature(tokens) {
			...
			protected Range rangeToExamine(ChunkedBinaryExtraction cbe) {
				...
				else return Range.fromInterval(0, index+1);
			}
		};
	}
	...
	public static ExtractionFeature anywhereAfterArg2(Collection<String> tokens) {
		return new TokenFeature(tokens) {
			...
			protected Range rangeToExamine(ChunkedBinaryExtraction cbe) {
				...
				else return Range.fromInterval(index, cbe.getSentence().getLength());
			}
		};
	}
	...
	/**
	 * Get a feature that fires if any element of tokens is present
	 * in a single token relation (returns empty range if relation is longer
	 * than one token).
	 * @param posTags
	 * @return
	 */
	public static ExtractionFeature relSingleToken(Collection<String> tokens) {
		return new TokenFeature(tokens) {
			...
			protected Range rangeToExamine(ChunkedBinaryExtraction cbe) {
				...
				if (range.getLength() == 1) return range;
