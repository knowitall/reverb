...
/**
 * An extractor that uses a regular expression pattern 
 * relations from NP-chunked sentences. This class uses a 
 * {@link LayeredTokenPattern} object to represent the regular expression 
 * pattern.
 * @author afader
 *
 */
public class RegexExtractor extends Extractor<ChunkedSentence, ChunkedExtraction> {
     ...
     * @param pattern the relation pattern
    ...
    public Collection<ChunkedExtraction> extractCandidates(
    	...
    	try {
	        ...
	        Collection<ChunkedExtraction> results = 
	        	new ArrayList<ChunkedExtraction>();
