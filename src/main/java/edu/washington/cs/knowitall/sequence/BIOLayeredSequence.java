 ...
 * {@code O B-X I-X O B-Y O B-Z B-Z I-Z} represents four spans at 
 * positions 1-2, 4, 6, and 7-8. 
 * @author afader
 *
 */
public class BIOLayeredSequence extends SimpleLayeredSequence {
    ...
    /**
     * Returns the ranges of all of the spans on the given layer that are
     * of the given type (e.g. will return all B-X/I-X given X).
     * @param layerName
     * @param type
     * @return
     */
    public ImmutableCollection<Range> getSpans(String layerName, String type) {
        if (hasLayer(layerName)) {
            if (spans.containsKey(layerName) && spanTypes.get(layerName).containsKey(type)) {
     ...
     * @param layerName 
     * @param input
     * @throws SequenceException if unable to add a layer with the given name,
     * or if input does not follow the B/I/O encoding.
     */
    public void addSpanLayer(String layerName, List<String> input) 
        throws SequenceException {
        
        ImmutableMultimap<String, Range> typeToSpans = ImmutableMultimap.copyOf(getRanges(input));
     ...
     * tag and the B/I/O encoding. For example, if <code>tag = "NP"</code> 
     * then this will add <code>B-NP</code>, <code>I-NP</code>,
     * and <code>O</code> tags at the indexes covered by <code>ranges</code>.
     * The ranges must not overlap.
     * @param layerName
     * @param tag
     * @param ranges
     * @throws SequenceException if any of the layers overlap, or if
     * any of the ranges are outside of the range of this sequence
     */
    public void addSpanLayerRanges(String layerName, String tag, 
        ...
        for (int i = 0; i < getLength(); i++) sequence.add("O");
    ...
    /**
     * Returns a subsequence of the given layer. If the given layer
     * is a span layer with B/I/O tags, and the subsequence partially 
     * intersects a span (i.e. it starts with I-X), then this initial tag
     ...
     * partially intersects a span (e.g. the subsequence starts at a 
     * I-X tag), then it will be replaced with a B-X tag.
     */
    ...
    private TreeMultimap<String, Range> getRanges(List<String> tags) throws SequenceException {
        ...
        for (int i = 0; i < tags.size(); i++) {
            ...
            } else if (tag.startsWith("I-") && inType != null && tag.equals("I-" + inType)) {
