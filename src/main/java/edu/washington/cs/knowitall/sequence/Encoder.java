 ...
 * It is used by {@link LayeredTokenPattern} for matching patterns against
 * {@link LayeredSequence} objects. 
 * </p>
 * <p>
 * The core of this class is a mapping from string tuples of length {@code n}
 * to integers {@code 0 <= i <} {@link Encoder#MAX_SIZE}. The mapping is defined 
 * by a list of {@code n} sets of String symbols {@code S_1, ..., S_n}, and a 
 * special symbol {@link Encoder#UNK}. The mapping assigns an integer value to each 
 * tuple {@code (x_1, ..., x_n)}, where {@code x_i} is either in {@code S_i} or is
 * the symbol {@code UNK}. For example, if {@code n = 2} and {@code S_1 = S_2 =
 * {0,1}}, then a possible mapping would be {@code (0,0) => 0, (0,1) => 1, 
 ...
 * {@code (|S_1|+1) * (|S_2|+1) * ... * (|S_n| + 1)} must be less than or 
 * equal to 2^16.
 * </p>
 * @author afader
 *
...
public class Encoder {
	...
	/**
	 * Constructs a new encoding table using the given symbol sets. These 
	 * symbol sets should not contain the unknown symbol {@link Encoder#UNK}. 
	 * @param symbols
	 * @throws SequenceException if the symbol sets result in an encoding table
	 * larger than {@link Encoder#MAX_SIZE}.
	 */
	public Encoder(List<Set<String>> symbols) throws SequenceException {
		...
		encodingTable = new HashMap<StringArrayWrapper, Integer>(encodingTableSize);
		int i = 0;
	...
	public char encode(String[] tuple) throws SequenceException {
		...
		for (int i = 0; i < tuple.length; i++) {
			String val = tuple[i];
			if (val.equals(UNK)) {
				String tupleStr = "(" + StringUtils.join(tuple, ", ") + ")";
				String msg = String.format(
					"Symbol at position %s in %s equals %s", i, tupleStr, UNK);
				throw new SequenceException(msg);
			}
		}
	...
	/**
	 * Encodes the given tuple as a char. This method assumes that the tuple
	 * has already had the unknown symbols mapped to UNK.
	 * @param tuple
	 * @return the encoding
	...
	private char encodeMapped(String[] tuple) throws SequenceException {
		...
		if (encodingTable.containsKey(wrappedTup)) {
			...
			String tupStr = "(" + StringUtils.join(tuple, ", ") + ")"; 
			System.out.println("---------------");
	 ...
	 * @param index the position in the tuple (defined by the order of sets
	 * passed to the constructor)
	 * @param value
	 * @return the encoding as an array
	 * @throws SequenceException if the index is out of bounds, or if any of
	 * the resulting tuples cannot be encoded
	 */
	public char[] encodeClass(int index, String value) 
	...
	/**
	 * Maps the given tuple to an intermediate representation, where any
	 * symbols that did not appear in the sets provided to the constructor
	 * to the {@link Encoder#UNK} symbol.
	 * @param tuple
