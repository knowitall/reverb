 ...
 * {@link LayeredTokenPattern} class. A pattern consists of two types
 * of tokens: layer/symbol pairs like {@code The_pos}, and meta-characters
 * defined by {@link java.util.rexex.Pattern}. This class takes a string
 * consisting of these and breaks them into tokens.
 * @author afader
 *
 */
public class LayeredPatternTokenizer {
	...
	private final String tokenPatternStr = 
	...
	public LayeredPatternTokenizer() {
		...
		for (String metaChar : metaChars) metaCharSet.add(metaChar);
	}
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (isMetaChar(token)) {
				...
				String msg = String.format(
					...
					+ "layer/symbol pair or a meta-character", text, token);
