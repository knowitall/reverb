...
/**
 * This class represents a field of a {@link SpanExtraction} that has
 * been normalized in some way, e.g. morphological normalization. A normalized
 * field is a {@link SimpleLayeredSequence} with two layers: a token layer, and
 * a POS layer. The token layer contains the normalized words. The POS layer
 * contains the POS tags for those words. 
 * @author afader
 *
 */
public class NormalizedField extends SimpleLayeredSequence {
	...
	public String getTokensAsString() {
		return StringUtils.join(getTokens().iterator(), " ");
	}
	...
	public String getPosTagsAsString() {
		return StringUtils.join(getPosTags().iterator(), " ");
	}
