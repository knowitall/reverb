 ...
 * a single object that converts String sentences that are all capitalized letters
 * to {@link ChunkedSentence} objects. 
 * 
 * @author schmmd
 * 
 */
public class OpenNlpCapitalSentenceChunker extends OpenNlpSentenceChunker {
    ...
    public OpenNlpCapitalSentenceChunker() throws IOException {
    	super(
			...
			new POSTaggerME(new POSModel(OpenNlpCapitalSentenceChunker.class.getResourceAsStream(taggerModelFile))),
			new ChunkerME(new ChunkerModel(OpenNlpCapitalSentenceChunker.class.getResourceAsStream(chunkerModelFile))));
    }
}
