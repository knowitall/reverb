...
public class OpenNlpSentenceChunker implements SentenceChunker {
     ...
     * {@link DefaultObjects}.
     * @throws IOException if unable to load the models.
     */
    ...
    public ChunkedSentence chunkSentence(String sent) throws ChunkerException {
        ...
        if (attachOfs) OpenNlpUtils.attachOfs(tokens, npChunkTags);
        if (attachPossessives) OpenNlpUtils.attachPossessives(posTags, npChunkTags);
        
        ChunkedSentence result = new ChunkedSentence(tokens, posTags, npChunkTags);
        return result;
    }
}
