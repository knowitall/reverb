 ...
 * read <code>NpChunkedSentence</code> objects from a <code>BufferedReader</code>. This object uses
 * the <code>SentenceExtractor</code> to obtain <code>String</code> sentences from the input, and then
 * chunks the sentences using the <code>SentenceChunker</code> object.
 * @author afader
 *
 */
public class ChunkedSentenceReader implements Iterable<ChunkedSentence> {
     ...
     * default <code>OpenNlpSentenceChunker</code> object.
     * @param r
     * @param se
     * @throws IOException
     */
    public ChunkedSentenceReader(Reader r, SentenceExtractor se) throws IOException {
    	if (SENT_CHUNKER == null) SENT_CHUNKER = new OpenNlpSentenceChunker();
    ...
    private void init(Reader reader, SentenceExtractor sentExtractor, SentenceChunker sentChunker) {
        ...
        SentenceBlocksIterator sbi = new SentenceBlocksIterator(tbi, sentExtractor);
